...
class Board(object):  # maybe we'll need to inherit Array
	...
	def add_history(self):
		...
		if self.states.items():
			#print(list(zip(*self.states.items())))
	...
	def do_move(self, move):
		...
		# do the move & check its legality
		legal = self.check_move_legal(move)
		...
		if legal:
			...
			# restore the self.states & self.availables
			self.states = self.states_history[-1]
			self.availables = copy.deepcopy(self.availables_history)
			return True # The player needs to make a move again

	def check_move_legal(self, move):
		"""Do the move: changes self.states, self.availables
		returns whether this move is legal or not"""
		...
		if num_taken > 1:
			...
			if self.states == self.states_history[-2]:
				return False
			else:
				self.availables.discard(move)
				return True
	...
	def count_go_number(self):  # needs to be examined
		...
		availables = list(self.availables)
		states = self.states
		for move in availables:
			...
			if list(border) == [self.players[0]]:
				for move_bd in blank_area:
					states[move_bd] = self.players[0]
					availables.remove(move_bd)

			elif list(border) == [self.players[1]]:
				for move_bd in blank_area:
					states[move_bd] = self.players[1]
					availables.remove(move_bd)
			elif list(border) == []:
				# empty board
				for move_bd in blank_area:
		...
		# count the number
		num = list(states.values())
		num_player1 = num.count(self.players[0])
		num_player2 = num.count(self.players[1])
	...
	def game_end(self, is_shown):
		...
		if len(self.availables) > 0.5*self.height*self.width:
			return False, None
		...
		if is_shown:
			if winner != None:
				...
				print("Go number: player%s, %s" %(self.players[0], num_player1))
				print("Go number: player%s, %s" %(self.players[1], num_player2))
			else:
				print("Game end. Tie")
		return True, winner
...
class Game(object):
	...
	def start_play(self, player1, player2, is_shown=1): # what is  inside the player1?
		...
		while True:
			...
			end, winner = self.board.game_end(is_shown)
			if end:
				return winner
	...
	def start_self_play(self, player, is_shown=0, temp=1e-3):
		...
		while True:
			...
			end, winner = self.board.game_end()  # REVISEMENT NEEDED
