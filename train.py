...
class TrainPipeline():
	def __init__(self, init_model=None):
		...
		self.kl_targ = 0.02
		self.check_freq = 10
		self.game_num = 1500  # number of games for self-play
	...
	def policy_evaluate(self, n_games=10):
		...
		for i in range(n_games):
			winner = self.game.start_play(current_mcts_player,
										  old_mcts_player,
										  is_shown=1)
			# winner =1 : current mscts ;  winner = 2 : old mcts; winner = None : Tie
			if winner:
				win_cnt[winner - 1] += 1
