...
class PolicyValueNet():
	def __init__(self, board_width, board_height,
				 steps_considered, model_file=None):
		...
		self.input_states = tf.placeholder(
			tf.float32, shape=[None, steps_considered + 1, board_height, board_width]
		)
	...
	def policy_value_fn(self, board): # the policy_value_fn takes game.board as input
		...
		current_state = np.ascontiguousarray( # send game.board.current_state() to the network input
			board.current_state(self.steps_considered).reshape(
				-1, self.steps_considered + 1, self.board_width, self.board_height)
		)
